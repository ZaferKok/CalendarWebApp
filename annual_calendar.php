...
if(!isset($_GET["selectedMonth"]))
{
	...
	echo "<div class='title'><h1>$year Calendar and Appointments</h1></div>";
	...
	echo "<div style='background-color:#7bed9f; width:100px; height:25px; display:inline-block; color:black;'>next apps</div>";	
	echo "<div style='background-color:gray; 	width:100px; height:25px; display:inline-block; color:white;'>old apps</div>";	
...
function build_calendar($month, $year, $calendar, $savedAppointments) 
{
	...
	$calendar  .= "<caption>
	<h4 style='display: flex;'><label class='kleinTableTitle'><a href='?seite=appointment_month&selectedDate=$month-$year'>$monthName $year</a></label></h4>
				  </caption>";
	...
	if($dayOfWeek > 0) {$calendar .= "<td colspan=$dayOfWeek></td>";}
		...
		while($currentDay <= $numberOfDaysInMonth)
		{
			...
			else if($currentDay == $AppointmentsOfThisMonth[$month][$currentDay])
			{
				...
				{
					$calendar .= "<td class='appointmentDay' style='background-color:gray; color:white;'>$currentDay</td>";
				}else 
				{
